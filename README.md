...
### API Key Issues
- Ensure both `OPENROUTER_API_KEY` and `OPENAI_API_KEY` are set in `.env`
- Check API key validity at the respective provider websites
- Verify `FORCE_OPENROUTER=true` if you want to use OpenRouter for all LLM calls

### Upload Issues
- Ensure PDF files are not corrupted or password-protected
- Check file size limits (default FastAPI limit applies)

### Retrieval Issues
- Verify that PDFs have been successfully uploaded and processed
- Check that the vector database contains documents (use `/api/status` endpoint)
- Try rephrasing your question if no relevant results are found
